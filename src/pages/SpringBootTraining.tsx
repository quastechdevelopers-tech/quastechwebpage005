...
const SpringBootTraining = () => {
  ...
  const modules = [
    { title: "Spring Boot Basics", topics: ["Spring Framework", "Spring Boot Setup", "Auto Configuration", "Dependency Injection"] },
    { title: "REST APIs", topics: ["RESTful Services", "Controllers", "Request Mapping", "JSON Processing"] },
    { title: "Distributed Systems Basics", topics: ["Service Communication", "Resilience Patterns", "API Gateway Basics", "Load Handling"] },
    { title: "Database Integration", topics: ["Spring Data JPA", "Hibernate", "MySQL", "MongoDB"] }
  ];

  return (
    <DarkModeProvider>
      <SEO title="Spring Boot Course - REST API Development Training" description="Learn Spring Boot framework to build robust RESTful APIs and enterprise applications." keywords="spring boot, spring framework, REST API, Java backend" canonical="/spring-boot-training" ogImage="/images/course images/java.jpg" category="IT Courses" />
      <CourseStructuredData name="Spring Boot Development" description="Spring Boot training for modern Java applications." url="https://quastech.co.in/spring-boot-training" duration="P4M" courseMode="blended" image="https://quastech.co.in/images/course images/java.jpg" />
      <BreadcrumbStructuredData items={[{ name: "Home", url: "/" }, { name: "Courses", url: "/#courses" }, { name: "Spring Boot", url: "/spring-boot-training" }]} />
      <div className="min-h-screen bg-gradient-to-br from-background via-muted/30 to-background">
        ...
        <section className="relative w-full min-h-[600px] md:h-[600px] flex items-center overflow-hidden py-8 md:py-0" style={{ background: 'linear-gradient(135deg, #3b82f6 0%, #f97316 100%)' }}>
          ...
          <div className="relative z-10 container mx-auto px-4"><div className="text-white space-y-6"><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}><Link to="/"><Home className="w-4 h-4 inline" /> Home</Link><ChevronRight className="w-4 h-4 inline" /><span>Spring Boot</span></motion.div><h1 className="text-5xl font-bold">Spring Boot Development</h1><p className="text-xl">Build robust REST APIs</p><Button className="bg-white text-blue-600">Enroll Now</Button></div></div>
        </section>
        ...
        <CourseTechnologySection courseSlug="spring-boot-training" />
        <BranchSection />
        <Footer />
      </div>
    </DarkModeProvider>
...
export default SpringBootTraining;
